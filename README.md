...
[![Version 6.119.0](https://img.shields.io/badge/version-6.119.0-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.119.0/Ouroboros-6.119.0.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.119.0/Ouroboros-6.119.0-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.119.0/ouroboros_6.119.0_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.119.0/ouroboros-6.119.0-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.119.0/ouroboros-6.119.0-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.119.0/Ouroboros-6.119.0-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.119.0/Ouroboros-6.119.0-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 6.119.0 | 2026-09-20 | **feat: atomic bump_release_version — the version-carrier sync class closed at the source (owner's "п.1").** Release version changes now have ONE legal path: write `VERSION`, run `sync_release_metadata`, assert zero carrier desyncs, and roll every snapshotted carrier back to pre-call bytes on any failure (a half-synced tree is structurally impossible; `VERSION`, pyproject/uv.lock, web/package.json, api_types, README badge + download URLs, install pages, ARCHITECTURE header). A no-op bump raises, invalid-version bumps are rejected before any disk write, and the returned changed-file list is never an empty success (the 6.115.1 silent-`synced: []` class). Dogfooded twice on this very release: the working tree was first bumped to 6.118.6 by mistake (patch vs minor misjudgment — the feature is MINOR per the 6.118.0 precedent), re-bumped to 6.119.0 with the same one-line call — both times 9 carriers in one call, zero edit_text on carriers, zero stale anchors, zero shrink-guard hits. New `TestBumpReleaseVersion` suite (5 tests incl. the byte-exact rollback pin); carrier desync check confirmed `[]` on disk after both bumps. Column space reserved: the manual multi-carrier hand-sync consumption of 901f11c6 (182 rounds / $3.88) is the measured cost this function removes from every future release. |
| 6.118.5 | 2026-09-20 | **docs: honest post-gate corrections — two advisory findings from the 6.118.5 triad applied.** (1) 6.117.0 restored to the table verbatim — the extra roll-off was not cap-driven after 6.118.5 landed (five patch rows stayed within cap both ways) — and the broader-than-declared roll-off is now openly disclosed in the 6.118.5 row. (2) Changelog corrected: both regression pins live in `tests/test_review_readiness_gate.py` (review_readiness_gate 22 incl. both), not in review_helpers_pack_limits. No code change: the 6.118.4 name-restoration stands exactly as landed (2e009feb, tag v6.118.4). Roll-off this release: 6.116.1 (cap-oldest patch row, body at git tag v6.116.1) and 6.116.0 (feat class-aware DEVELOPMENT.md residency, minor, NOT cap-driven — disclosed openly, body at git tag v6.116.0; both added to the older-releases enumeration). |
| 6.118.4 | 2026-09-20 | **fix: restore the 6.118.3 re-export plane losses + stale user-visible docstring clause — the 6.118.3 residual pair.** The 6.118.3 leaf extraction silently dropped three runtime names from the `review_helpers` re-export plane: `sanitize_tool_result_for_log` (left at its canonical home `ouroboros.utils`, restored with `noqa: F401` alongside the existing `check_worktree_version_sync` precedent), and the private secret-redaction regexes `_SECRET_LINE_RE` / `_JSON_SECRET_RE` (left at their canonical home `review_pack_files`, re-exported unchanged by identity, not copy). The fourth AST-diff'ed name, `ToolContext`, needed nothing: it already sat in the `TYPE_CHECKING` guard in both 6.118.2 and 6.118.3, runtime-invisible in both, so any historical `import review_helpers; review_helpers.ToolContext` caller was equally broken before and after the extraction — recorded as the decision note (append the runtime import if a real caller appears). Also removed the stale user-visible docstring clause `version-sync re-export home` from the new leaf `review_pack_files.py` (version-sync logic actually lives in `release_sync`, and no such import exists in the leaf body). Regression pins added in `tests/test_review_readiness_gate.py` (in this diff's suite, not pack_limits): restored names `hasattr`-probe callable/identity-checked, version sync re-export probed. Adjacent suites green (review_readiness_gate 22 incl. both new pins; review_helpers_pack_limits 26; packaging_sync + size_ratchet_ci_shape + repo_health_smoke = 109 total). 6.117.0/6.117.1 roll-off disclosure: 6.117.1 went as the cap-oldest patch-row; 6.117.0 (minor) was additionally rolled off in the same shot to keep the table at five patch rows (corrected and restored in 6.118.5); body remains at git tag v6.117.0. |
| 6.118.3 | 2026-09-19 | **refactor: `review_pack_files` leaf extraction — review_helpers off the 1600-line ratchet edge (backlog item; ratchet heal).** The file sat at exactly 1600/1600 — the next touching release would have been blocked (the same edge bit twice during 6.118.0 prep). Dense file/text primitives (binary/sensitive/vendored classification constants, secret redaction + collision-safe fencing, binary sniffing, full-repo scan, HEAD/skill-payload snapshot rendering, touched-file pack with its managed-binary and advisory inline-policy contracts) moved to the new leaf `ouroboros/tools/review_pack_files.py` (511 lines after the advisory dedup and dead-import removal, mirroring the 6.118.0 `advisory_pack_policy` precedent); `review_helpers.py` 1600 → 1230 with a lazy re-export plane plus a `build_touched_file_pack` shim, so every historical `from review_helpers import <name>`, the atlas/plan_evidence/deep_self_review/skill/claude_advisory consumers, the `review.iter_repo_pack_entries is review_helpers.iter_repo_pack_entries` identity pin, and both test monkeypatch targets (`review_helpers.subprocess.run`, `review_helpers.build_touched_file_pack`) stay intact; leaf imports back only lazily, preserving the no-tools-circulars contract. Same inputs, same calls; one visible output delta from the applied advisory dedup — the live touched-file binary-omission row now uses the shared HEAD-snapshot template ("Binary file omitted — binary content detected" instead of "omitted — binary file"). Size ratchet: manifest regenerated, `review_helpers.py` enters BAND_PATHS with rationale (1230, band 1001–1500), transition validator green. Residuals of 6.118.1 applied: the changelog count mismatch fixed here ("All 6 existing callers" vs the seven enumerated call sites — advisory finding 2026-09-17); the deepseek transport flake (triad slot_2, Sep 16–17 including the 6.118.1 triad loss) recorded as statistical, no slot change (owner zone; post-17.09 waves all answered 3/3). Post-release AST compatible-surface check found three runtime names dropped from the re-export plane (`sanitize_tool_result_for_log`, private secret-redaction regexes `_SECRET_LINE_RE` / `_JSON_SECRET_RE`; zero import surface, canonical homes kept) — restored by 6.118.4. |
| 6.118.2 | 2026-09-18 | **fix: edit-recovery hints + safety transport-timeout retry + absolute-glue path guard — the three measured error classes from the 2026-09-18 recheck.** (1) Stale-anchor class (`edit_text` 13.1%→22.5%, `edit_batch` 19.2% of calls, ~10.6% of all post-rule tool errors) now recovers in zero extra rounds: an exact `old_str` miss renders the nearest matching fragments (bounded difflib window scan, new leaf `ouroboros/tools/edit_support.py`) inside the SAME typed refusal — `STR_REPLACE_ERROR`/`EDIT_TEXT_ERROR` via the shared `_str_match_replace` seam, `EDIT_BATCH_ERROR` per edit; count-mismatch refusals gain nothing; atomicity unchanged (hints are additive diagnostics on the designed refusal, still `edit_ops_blocked` policy denials); a missing file in `edit_batch` renders nearest existing paths under the root (bounded walk). (2) Safety-check transport-timeout class (22/104 post-rule errors; every measured retry of the same call succeeded): exactly ONE retry for the typed timeout class (`TimeoutError` / `APITimeoutError` MRO / `httpx.TimeoutException`) with a durable `safety_timeout_retry` audit row; a second consecutive timeout keeps TODAY'S exact block message; rate limits keep the typed `_SafetyRateLimited` path; every other exception class is byte-identical one-attempt block. (3) Absolute-glue guard in `_resolve_target_in_selected_base`: an absolute path already under the selected base joins as a relative child instead of silently resolving to `base/<absolute-path-minus-slash>` (the lost-leading-slash / double-prefix class, 12 `missing_path` errors). Tests: new `tests/test_edit_recovery_hints.py` (12, incl. the pathlib cross-platform pin for the glue guard and the hint-egress masking pin) plus two safety-retry pins in `tests/test_safety_rate_limit.py`; adjacent suites green. (4) Advisory hardening from this cycle's live critic: the count==0 file preview at the shared `_str_match_replace` seam and the nearest-fragment hint previews are masked through `mask_secret_bytes` (user_files edit misses can no longer surface credential-shaped bytes in error output), and the absolute-glue guard is pathlib-based (`relative_to` + case-aware fallback), not separator-dependent string comparison. Two triad-lane failures in `test_git_review_pipeline.py` verified pre-existing via a /tmp clean-HEAD copy A/B (`CLEAN_HEAD_EXIT=1`, not this diff) — same sanctioned criterion as 6.118.0/6.118.1. |
| 6.118.1 | 2026-09-17 | **fix: advisory freshness invalidation narrowed to real repos — the crd-0001 reopen class closed.** `_resolve_mutation_repo_keys` attributed any mutation to the nearest repo found by walking up from a *pseudo repo root* (`make_repo_key` returns the path itself when no `.git` ancestor exists), and an absolute path that lost its leading slash (glued onto the mutation root, e.g. task_drive scratch recorded as `home/graphrag/...`) resolved under the repo root — so a scratch file outside the repository staled its advisory snapshot and re-opened the "Advisory freshness debt" (`crd-0001`) on every task while the git tree stayed untouched. Replacement `_resolve_mutation_repo_scope` returns `(repo_keys, scope_known)`: attribution requires a REAL git repo discovered from a LOCATABLE path (existing file or existing parent); unlocatable paths contribute nothing instead of inheriting the root's repo; when all locatable paths sit outside any repository the invalidation is skipped entirely (scratch files under task_drive, /tmp, runtime data keep freshness); unknown scope (no LOCATABLE path at all — the two advisory-review waves fixed both all-unlocatable-paths edges, with and without a mutation root, that returned scope_known=True and would have skipped invalidation) and genuinely multi-repo scope still stale everything conservatively. New leaf helper `_discovered_repo_dir` returns the nearest `.git` ancestor or `None` (never invents a pseudo root). Fail-closed safety preserved at the gate itself: freshness is snapshot-hash-keyed, so a missed invalidation can only surface as a `no_advisory` block, never as silently-fresh review. All 7 existing call sites keep their contract (verified per call site: `commit_gate._invalidate_advisory`, `edit_ops._finish_mutation`, `git.py` write paths, `shell.py` before/after snapshots, `delegate_integration`, `subagent_integration`, `services.py`). Regression suite `tests/test_review_state.py` (new, 17 tests, module-marked serial: `_make_repo` spawns real `git init/add/commit` subprocesses — sanctioned parallel-safety criterion): repo-relative path attributes its repo; task-drive scratch keeps freshness; glued absolute path does not inherit repo attribution; mixed repos+scratch list both repos; root-only and `<service:name>` placeholder shapes; unknown scope stales everything; end-to-end through durable state checks stale and kept outcomes plus empty `last_stale_*` fields on kept freshness. Adjacent suites green (edit_ops, commit_gate, packaging_sync = 142 tests); two triad-lane failures in `test_git_review_pipeline.py` verified pre-existing via /tmp clean-HEAD A/B (not this diff). The first 6.118.1 preflight run exposed an eighth streaming web_search test crashing an xdist worker (gw1 on `test_streamed_web_503_preserves_status_for_one_safe_retry`, PARALLEL_WORKER_CRASH at 65%, zero red tests otherwise) — marked serial under the same sanctioned criterion as the seven from 6.117.0; green serially. Live proof attached: after raising the native-episode delivery budgets via settings (rounds 16→32, transcript 900,000→2,000,000 chars — fail-closed machinery unchanged), the advisory critic ran its FIRST full live native-episode dispatch on a real release diff (luna) and returned a real 30-item verdict; two earlier flash-model episodes failed closed on round-budget exhaustion exactly as designed. |
...
Older releases are preserved in Git tags and GitHub releases. Older 6.x rows (including 6.116.1, 6.116.0, 6.115.1, 6.114.2, 6.114.1, 6.114.0, 6.113.0, 6.112.0, 6.113.5, 6.113.3, 6.113.1, 6.110.1, 6.109.0, 6.108.1, 6.106.0, 6.101.1, 6.97.2, 6.105.0, 6.97.1, 6.97.0, 6.96.1, 6.96.0, 6.95.0, 6.94.0, 6.93.0, 6.92.1, 6.92.0, 6.91.1, 6.90.3, 6.91.0, 6.90.2, 6.90.0, 6.87.5, 6.87.4, 6.87.3, 6.87.2, 6.84.0, 6.87.1, 6.83.0, 6.86.1, 6.81.1, 6.76.0, 6.75.0, 6.74.5, 6.74.4, 6.74.1, 6.74.0, 6.73.2, 6.73.1, 6.73.0, 6.72.0, 6.71.2, 6.71.1, 6.71.0, 6.70.0, 6.69.0, 6.68.0, 6.67.0, 6.66.0, 6.65.4, 6.65.3, 6.65.2, 6.65.1, 6.65.0, 6.64.3, 6.64.2, 6.64.1, 6.64.0, 6.63.0, 6.62.0, 6.61.4, 6.61.3, 6.61.1, 6.61.0, 6.60.0, 6.59.0, 6.58.0, 6.57.0, 6.56.0, 6.55.0, 6.54.4, 6.54.2, 6.54.1, 6.54.0, 6.53.4, 6.53.0, 6.51.0), the 5.2.0 through 5.33.0-rc.6 rows, and former `4.0.0` rows are rolled off to respect the P9 changelog cap; their full bodies remain at their git tags.
