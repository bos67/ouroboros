...
[![Version 6.119.19](https://img.shields.io/badge/version-6.119.19-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.119.19/Ouroboros-6.119.19.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.119.19/Ouroboros-6.119.19-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.119.19/ouroboros_6.119.19_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.119.19/ouroboros-6.119.19-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.119.19/ouroboros-6.119.19-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.119.19/Ouroboros-6.119.19-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.119.19/Ouroboros-6.119.19-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 6.119.19 | 2026-09-26 | **fix: changelog count correction for the 6.119.18 row (triad advisory, disk-adjudicated) + a gate pin closing the numbers-from-memory class.** The .18 row shipped with prose counts (22/22 = 14+8 serial; neighbors 126/126) while its own tree carried 21 collected tests and 170 collected neighbors — the triad caught both after commit, the correction was staged as the candidate patch `readme_count_fix_611919_candidate.patch` when .18's budget closed, and lands here: the row now quotes the machine (21/21 = 14 parallel + 7 serial; 170/170 collected), re-derived live on this tree. The class fix is `tests/test_changelog_counts_pin.py` (new, 5 tests): the NEWEST Version History row's canonical count claims — a lane claim `NN/NN green across both marker lanes (P parallel + S serial)` or a neighbor-sum claim `= NN/NN collected and passed` — are re-derived by a live `pytest --collect-only` (the same marker-lane split CI and the hermetic gate run) and any mismatch fails the release before commit; historical rows are frozen tag truth and never re-checked; a claim naming a suite that does not exist fails rather than skips. A/B proof on this tree: RED with the exact diff in the failure message (`row claims 126 ... tree collects 170`) when the as-shipped .18 wording was temporarily restored, GREEN on the corrected row. Verification: `tests/test_changelog_counts_pin.py` 5/5 green across both marker lanes (3 parallel + 2 serial); neighbors green by exit code (`test_release_sync` + `test_packaging_sync` + `test_docs_sync` = 124/124 collected and passed). Roll-off this release: 6.119.14 (cap-oldest patch row; six patch rows exceeded the P9 cap of five after this row was added — cap-driven; body at git tag v6.119.14, added to the older-releases enumeration). |
| 6.119.18 | 2026-09-24 | **test: discriminating selector-cwd pin for the shell-preflight hint (6.119.17 triad residual, closed without a second paid gate).** The 6.119.17 gate's own review flagged that the shipped spy test passes cwd as an ABSOLUTE path, so it also passes unchanged against the pre-fix `.16` code — a test that passes under both hypotheses pins nothing. The discriminator passes cwd as the SELECTOR spelling `task_drive`: the binding layer resolves it to `drive_root/task_drives/<task_id>`, so the preflight audit must receive that resolved absolute dir — never the raw selector string, never the repo_dir fallback (both pre-fix behaviors). A/B verified on this tree before staging: RED against `git show v6.119.16:ouroboros/tools/shell.py` (temporarily restored, byte-identical re-restore verified by sha256), GREEN on `.17` code; the healthy-path leg still asserts the dispatch actually runs (no refusal). The test body is the artifact `selector_cwd_discriminator_test.patch` from task 803db648 applied verbatim. Verification: `tests/test_shell_preflight.py` 21/21 green across both marker lanes (14 parallel + 7 serial); neighbors green by exit code (`test_shell_run_shell` + `test_release_sync` + `test_packaging_sync` + `test_docs_sync` = 170/170 collected and passed, incl. the three-cell render pin that caught the missing changelog row before this commit); ruff clean on the touched file (155 repo-wide findings verified pre-existing by A/B against `git show HEAD:`); carriers 9/9 via `bump_release_version` in one call. Roll-off this release: 6.119.13 (cap-oldest patch row; six patch rows exceeded the P9 cap of five after this row was added — cap-driven; body at git tag v6.119.13, added to the older-releases enumeration). |
