...
[![Version 6.118.3](https://img.shields.io/badge/version-6.118.3-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.118.3/Ouroboros-6.118.3.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.118.3/Ouroboros-6.118.3-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.118.3/ouroboros_6.118.3_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.118.3/ouroboros-6.118.3-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.118.3/ouroboros-6.118.3-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.118.3/Ouroboros-6.118.3-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.118.3/Ouroboros-6.118.3-linux-x86_64.tar.gz
...
| 6.117.1 | 2026-09-16 | **fix: scope-review window evidence + release review findings.** Scope-pack assembly blocked every contract-bearing release on ~592K-token sizing; the root cause was the capability-evidence /models parser: sprutdock's catalog (`{total, items}` + `?offset=` pagination, 24/page) parsed as zero models, so reviewer routes fell to the conservative ~700K fallback. The parser now accepts root-list / `data` / `items` / `models` schemas, walks offset pagination (cap 40 pages), fails closed on unknown shapes; live-verified via the reviewer-window resolver (deepseek/glm confirmed 1,310,720; luna-pro owner-ack 1,000,000 kept). Scope slot moved to `deepseek-v4-flash-0731` (measured input limit 898,475 ≥ ~800K required — first contract-bearing release with a self-assembled scope pack since 6.114.0). Release review findings applied: ARCHITECTURE.md §7 preflight row updated to 2700s per-phase semantics (scope CRITICAL), budget-split docstrings/comments corrected 70%→80% to match `_PARALLEL_BUDGET_SHARE = 0.80`, serial-lane criterion comments added to the seven web_search streaming marks, node-expiry test uses the non-colliding pid convention + stubbed terminator, changelog count fixed to 1/16. |
