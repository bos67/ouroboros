...
[![Version 6.117.1](https://img.shields.io/badge/version-6.117.1-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.117.1/Ouroboros-6.117.1.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.117.1/Ouroboros-6.117.1-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.117.1/ouroboros_6.117.1_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.117.1/ouroboros-6.117.1-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.117.1/ouroboros-6.117.1-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.117.1/Ouroboros-6.117.1-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.117.1/Ouroboros-6.117.1-linux-x86_64.tar.gz
...
| 6.116.2 | 2026-09-14 | **fix: re-derive state `current_sha` from git HEAD on ordinary startup (ibl-current-sha-deriv, owner-ordered).** A manual commit plus a plain restart left `state.json.current_sha` stale (only update/checkout paths wrote it), which fed `/api/state` (stale ws.js reload-on-SHA kept the old UI after restarts), emitted `Worker SHA mismatch after spawn` on every spawn, and pinned the badge to an old release. Startup now heals the record once when no update/checkout intent is active (`supervisor.state.rederive_current_sha_from_repo` after the supervisor repo bootstrap, fail-safe on git errors, `current_sha_rederived` event). Size-ratchet manifest regenerated, closing crd-0003 and crd-0004: a living GIANT entry for `ouroboros/context.py` plus band rationales for `supervisor/state.py` (entered the band from 1023 lines after the fix) and `tests/test_project_routing_v664.py`. Residency classifiers moved to the leaf `ouroboros/context_residency.py` (`context.py` 1668 → 1570 lines, call sites unchanged, 37 doc/context tests pin the shape). |
