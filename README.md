...
[![Version 6.119.6](https://img.shields.io/badge/version-6.119.6-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.119.6/Ouroboros-6.119.6.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.119.6/Ouroboros-6.119.6-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.119.6/ouroboros_6.119.6_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.119.6/ouroboros-6.119.6-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.119.6/ouroboros-6.119.6-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.119.6/Ouroboros-6.119.6-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.119.6/Ouroboros-6.119.6-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 6.119.6 | 2026-09-22 | **fix: stale integrity facts cleared at the seam — the 6.119.5 triad critical closed.** The 6.119.5 stamp wrote its fact onto a REUSED usage dict (`prepare_terminal_send_event` receives the same usage dict that `terminal_result_fields` reads for the durable result), and its early-return code path left a PRIOR finalize's degenerate stamp in place on a later clean/short final of the same task: a pure-clean re-finalize kept the previous answer's `"degenerate": True` in the durable result — the immune system proving something about the wrong answer. The seam is the sole integrity writer, so it now OWNS both halves: when it re-derives, it stamps as before (write-wins); when it cannot re-derive (empty/short final, detector not applicable), it CLEARS the inherited fact — before the early returns, on every path, on every finalize. The clear path also honors the stamp's own text contract: a prior stamp embedded its frame sentence in the surviving text (the 6.119.5 stamp rewrites text today, so the shape is recognized exactly — derived regex pinned by a producer-minted recognize-its-own-format test), and the sole-writer cleanup removes only the stamped copy's frame — hand-authored or vendor prose is never touched. New `TestPriorStampCleanup` suite (5 tests: short/empty-final clears, clean re-finalize restates, recognize-own-format guard, salvage-copy frame-strip verified by reading the preserved bytes). Preflight+triad+scope green, carriers bumped 9-in-one-call via `bump_release_version` (no hand carrier edits, zero stale-anchor aborts). Roll-off this release: 6.119.0 (cap-oldest patch row, five rows exceeded P9 cap of five after this row was added — cap-driven; body at git tag v6.119.0, added to the older-releases enumeration). |
...
Older releases are preserved in Git tags and GitHub releases. Older 6.x rows (including 6.119.1, 6.119.0, 6.118.5, 6.118.4, 6.118.3, 6.118.2, 6.118.1, 6.116.1, 6.116.0, 6.115.1, 6.114.2, 6.114.1, 6.114.0, 6.113.0, 6.112.0, 6.113.5, 6.113.3, 6.113.1, 6.110.1, 6.109.0, 6.108.1, 6.106.0, 6.101.1, 6.97.2, 6.105.0, 6.97.1, 6.97.0, 6.96.1, 6.96.0, 6.95.0, 6.94.0, 6.93.0, 6.92.1, 6.92.0, 6.91.1, 6.90.3, 6.91.0, 6.90.2, 6.90.0, 6.87.5, 6.87.4, 6.87.3, 6.87.2, 6.84.0, 6.87.1, 6.83.0, 6.86.1, 6.81.1, 6.76.0, 6.75.0, 6.74.5, 6.74.4, 6.74.1, 6.74.0, 6.73.2, 6.73.1, 6.73.0, 6.72.0, 6.71.2, 6.71.1, 6.71.0, 6.70.0, 6.69.0, 6.68.0, 6.67.0, 6.66.0, 6.65.4, 6.65.3, 6.65.2, 6.65.1, 6.65.0, 6.64.3, 6.64.2, 6.64.1, 6.64.0, 6.63.0, 6.62.0, 6.61.4, 6.61.3, 6.61.1, 6.61.0, 6.60.0, 6.59.0, 6.58.0, 6.57.0, 6.56.0, 6.55.0, 6.54.4, 6.54.2, 6.54.1, 6.54.0, 6.53.4, 6.53.0, 6.51.0), the 5.2.0 through 5.33.0-rc.6 rows, and former `4.0.0` rows are rolled off to respect the P9 changelog cap; their full bodies remain at their git tags.
