...
[![Version 6.119.5](https://img.shields.io/badge/version-6.119.5-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.119.5/Ouroboros-6.119.5.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.119.5/Ouroboros-6.119.5-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.119.5/ouroboros_6.119.5_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.119.5/ouroboros-6.119.5-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.119.5/ouroboros-6.119.5-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.119.5/Ouroboros-6.119.5-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.119.5/Ouroboros-6.119.5-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 6.119.5 | 2026-09-21 | **feat: final-answer integrity stamp at the terminal delivery seam (owner's "п.1" quality direction, structural slice).** Three owner-facing final messages degraded into repetition-loop / fragment-insertion output (task 74c83c57's manifest-loop final measured trigram uniqueness 0.5686 with a top trigram repeating 27×), and the delivery seam carried those bytes to the owner with no typed fact anywhere — so recap pipelines and summaries could quote debris as my words. `prepare_terminal_send_event` now runs a deterministic shingle probe (`ouroboros.task_finalization.final_text_integrity_facts`) BEFORE its early returns, so every delivery path (live shortcut, buffered drain, outbox replay) carries it: measured facts — trigram uniqueness, top shingle, repeat count — ride `usage["final_text_integrity"]` and the durable result via `terminal_result_fields` (degenerate-only, additive), while degenerate finals additionally stamp `progress_meta["final_text_integrity"]` on the delivered frames — write-wins by the sole-writer seam (a pre-existing differing stamp is overwritten, other `progress_meta` keys untouched). DELIVERY NEVER MUTATES AN ANSWER: text leaving the door stays byte-exact; the seam only stamps — and a non-degenerate final produces no frame stamp; degenerate=false persists only as a usage-side fact. Thresholds measured, not tuned by feel: degenerate-incident window (0.569 uniqueness / 26–27× repeats) vs clean finals (0.986–0.997 / 2–5×, measured on real finals 1ae96feb, 451177be, 7f4ed83b); detector floor 60 words keeps short finals out of scope. The integrity key joins the browser ABI per §11.3: `ChatOutbound.final_text_integrity` (frozen owner), the `api_types.js` mirror, the §11.1 table entry, and the progress-meta allowlist member in `gateway/history.py` — pinned by `tests/test_gateway_parity.py`, `tests/test_contracts.py`, and the new suite. New suite `tests/test_final_text_integrity.py` (14 tests: corpus-shaped degenerate flag, per-branch OR isolation, applicability floor, ephemeral-path stamp, write-wins stamp, clean-no-visible-change pins, allowlist pin) — green; the live calibration run (script + report artifact) reproduced the gap between the measured incident and all three clean finals. Disclosed residual (measured, deliberate): 7f4ed83b's fragment-insertion class (foreign-language debris fused mid-sentence WITHOUT repetition) does NOT trip the repetition detector — classed separately, not silently claimed as covered by this fix. |
| 6.119.4 | 2026-09-21 | **docs: the 6.119.3 triad advisory applied — numeric-claim drift resolved with named measurement runs.** Four surfaces of the 6.119.3 diff stated the same removed-cost operation with two different figures (~107ms vs ~99ms, ~8% apart) without distinguishing the runs. All four (README row, `usage_ledger.py` comment, ARCHITECTURE.md ledger paragraph, test docstring) now carry both measurements with named provenance: 106.9ms in the release A/B at 17,320 rows and 99.2ms in the first same-day probe at 17,242 rows (~8% run-to-run spread, same operation). No code change; the 6.119.3 perf fix stands exactly as landed. Roll-off this release: 6.118.4 (cap-oldest patch row; six patch rows exceeded the P9 limit of five after this row was added — cap-driven; body at git tag v6.118.4, added to the older-releases enumeration). |
| 6.119.3 | 2026-09-21 | **perf: tail-only append validation in the usage ledger — the per-append whole-history revalidation removed from the monetary lock (owner's "hot-store" item, revised).** `_append_rows_locked` revalidated the ENTIRE history on every append: 106.9ms under the 45s monetary lock at 17,320 rows in the release A/B (99.2ms in the first same-day probe at 17,242 rows — ~8% run-to-run spread), on every reservation/settlement. The history is now trusted as input (every caller receives it from a validated read — full replay or the warm cache whose merges passed tail validation) and the append validates the TAIL through the documented additive `start_seq`/`states` resume seam: a cheap one-pass dense-sequence scan of the history (16.3ms, catches gappy caller arguments with the same exception class) plus tail transition validation against the full per-attempt state map. Measured A/B on a live-ledger copy: validation 106.9ms → 16.3ms (6.6×), full append path incl. JSON serialization + fsync 22ms; guarantee strength unchanged — a property test pins accept/reject/error-message identity vs whole-history validation across legal/illegal/gappy/resurrection cases, plus end-to-end public-API pins. The row-dropping compaction remediation named by the hot-store invariant was measured and is DEAD: every attempt already sits at its minimal validator-legal transition chain ({2: 26, 3: 5738} rows/attempt on the live ledger) — dropping non-final rows of terminal attempts is structurally impossible without weakening validator guarantees, so the remediation text now names the real residual (cold full replay, grows linearly) instead. Health-invariant text and ARCHITECTURE.md ledger-paragraph synced. Roll-off this release: 6.118.3 (cap-oldest patch row; six patch rows exceeded the P9 limit of five after this row was added — cap-driven; body at git tag v6.118.3, added to the older-releases enumeration). |
...
Older releases are preserved in Git tags and GitHub releases. Older 6.x rows (including 6.119.1, 6.118.4, 6.118.3, 6.118.2, 6.118.1, 6.116.1, 6.116.0, 6.115.1, 6.114.2, 6.114.1, 6.114.0, 6.113.0, 6.112.0, 6.113.5, 6.113.3, 6.113.1, 6.110.1, 6.109.0, 6.108.1, 6.106.0, 6.101.1, 6.97.2, 6.105.0, 6.97.1, 6.97.0, 6.96.1, 6.96.0, 6.95.0, 6.94.0, 6.93.0, 6.92.1, 6.92.0, 6.91.1, 6.90.3, 6.91.0, 6.90.2, 6.90.0, 6.87.5, 6.87.4, 6.87.3, 6.87.2, 6.84.0, 6.87.1, 6.83.0, 6.86.1, 6.81.1, 6.76.0, 6.75.0, 6.74.5, 6.74.4, 6.74.1, 6.74.0, 6.73.2, 6.73.1, 6.73.0, 6.72.0, 6.71.2, 6.71.1, 6.71.0, 6.70.0, 6.69.0, 6.68.0, 6.67.0, 6.66.0, 6.65.4, 6.65.3, 6.65.2, 6.65.1, 6.65.0, 6.64.3, 6.64.2, 6.64.1, 6.64.0, 6.63.0, 6.62.0, 6.61.4, 6.61.3, 6.61.1, 6.61.0, 6.60.0, 6.59.0, 6.58.0, 6.57.0, 6.56.0, 6.55.0, 6.54.4, 6.54.2, 6.54.1, 6.54.0, 6.53.4, 6.53.0, 6.51.0), the 5.2.0 through 5.33.0-rc.6 rows, and former `4.0.0` rows are rolled off to respect the P9 changelog cap; their full bodies remain at their git tags.
