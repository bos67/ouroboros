...
|------|----------|-----------------|----------------|
| Main task context (`context.py`) | full tier-0 | class-scoped residency in Max (self-body classes full; direct-chat and externally-bound classes get the lossless navigation map + pointer); lossless navigation map in Low | inclusion mode-independent (repo binding); owner-max rendered form class-scoped (v6.116.0): full for pooled/evolution/self-body work; direct-chat turns get the lossless nav map + pointer; visible on-demand pointer for a bound external workspace, subagent, or API/CLI/scheduled external surface. `workspace="none"` and explicit self-body overrides retain full Development. |
...
**Context mode (Low / Max).** `OUROBOROS_CONTEXT_MODE` controls the Architecture projection in the agent's own context: Max is class-scoped (v6.115.0, owner decision) — `ARCHITECTURE.md` is full-resident for self-body classes (default-lane pooled tasks, evolution, self-body work), while direct-chat turns and externally-bound work (external workspaces, project trees, subagents, API/CLI/scheduled surfaces) receive its lossless H2-H4 navigation map plus a visible on-demand pointer; Low supplies the navigation map for every class. `DEVELOPMENT.md` inclusion follows the active repository binding (mode-independent); its owner-max rendered form is class-scoped too (v6.116.0, owner decision — the ARCHITECTURE mirror): when the task targets Ouroboros's system repository the handbook is full for self-body and evolution work, while direct-chat turns receive its lossless H2-H4 navigation map plus a visible on-demand pointer; a bound external workspace, auto-provisioned external project tree, subagent, or API/CLI/scheduled external surface keeps the visible on-demand pointer. Low mode renders the full handbook for every inlined class. Explicit structured overrides remain authoritative (`context_requires_development=False` stays pointer-only, never nav). Tier-0 identity and constitutional context stays full in every mode.
