...
BAND_PATHS = {
    ...
    "ouroboros/task_pacing.py": "Self-check reminder scaffold moved from loop.py (6.119.12 shrink-authority release): pacing-text SSOT sibling of build_time_budget_note; band re-entry is the relocation artifact, shrink next touch.",
    ...
    "ouroboros/tools/review_helpers.py": "Entered the band from 1600: the 6.118.3 leaf extraction (review_pack_files.py) moved the dense file/text primitives out and left a re-export/shim plane; band re-entry is the reduction artifact, shrink on next touch.",
    ...
    "supervisor/state.py": "current_sha re-derivation at startup (ibl-current-sha-deriv fix) joined the state authority; band re-entry is the fix's artifact, shrink on next touch.",
    ...
    "tests/test_project_routing_v664.py": "v664 project-routing regression suite crossed 1000 lines with the routing batch; one coherent surface, split when a natural seam appears.",
