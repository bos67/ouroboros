...
sanctioned exceptions are the compatibility re-export plane: ``release_sync``
(``check_worktree_version_sync``), ``advisory_pack_policy`` (advisory
inline-policy constants, 6.118.0) and ``review_pack_files`` (dense file/text
pack primitives, 6.118.3).
"""
...
# Dense file/text primitives (binary/sensitive/vendored classification,
# secret redaction, collision-safe fencing, binary sniffing, full-repo
# scanning, HEAD snapshot rendering) live in the review_pack_files leaf since
# 6.118.3; this re-export plane keeps every legacy
# ``from review_helpers import <name>`` call site working, and
# review.iter_repo_pack_entries is review_helpers.iter_repo_pack_entries stays
# pinned by tests/test_repo_health_smoke.py.
from ouroboros.tools.review_pack_files import (  # noqa: E402
    BINARY_EXTENSIONS,
    _FILE_SIZE_LIMIT,
...
# redact_prompt_secrets / _make_fence / format_prompt_code_block live in the
# review_pack_files leaf (6.118.3); re-exported unchanged from the names above.
...
def build_touched_file_pack(
    ...
    *,
    ...
    """Shim: implementation lives in ``review_pack_files`` (6.118.3) with the
    inline_policy / managed-binary contract unchanged; this pass-through keeps
    the historical import time and the monkeypatch target
    ``ouroboros.tools.review_helpers.build_touched_file_pack`` stable."""
    from ouroboros.tools.review_pack_files import build_touched_file_pack as _impl
    return _impl(
        repo_dir,
        paths,
...
# build_head_snapshot_section lives in the review_pack_files leaf (6.118.3);
# re-exported unchanged from the names above.
