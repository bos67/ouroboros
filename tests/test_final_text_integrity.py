"""Final-text integrity gate at the terminal delivery seam (v6.119.7).
...
class TestPriorStampCleanup:
    ...
    2026-09-21). The seam is the sole integrity writer of BOTH surfaces —
    the usage fact and the progress_meta stamp — so the clear path wipes
    both; text is never touched (the stamp never mutates the answer).
    """

    def test_short_final_over_prior_degenerate_stamp_clears_both_surfaces(self, tmp_path):
        usage: dict = {"terminal_origin": "model_final"}
        ...
        event = {
            "type": "send_message", "task_id": "tc", "chat_id": 1,
            "text": "Short final.",
            "progress_meta": {"final_text_integrity": {"degenerate": True}},
        }
        prepare_terminal_send_event(
            tmp_path, {"id": "tc"}, "Short final.",  # < 60 words → probe not applicable
            usage, event, ephemeral=False, presence=False,
        )
        assert "final_text_integrity" not in usage
        assert "final_text_integrity" not in event["progress_meta"]
        assert event["text"] == "Short final."  # delivered text byte-exact
    ...
    def test_clean_final_over_prior_stamp_restates_facts(self, tmp_path):
        ...
        # The stale degenerate frame stamp is cleared with the fact: the
        # seam owns both surfaces; a clean final shows no integrity stamp.
        assert "final_text_integrity" not in event["progress_meta"]

    def test_delivered_text_is_never_touched(self, tmp_path):
        corpus = _loop_corpus()
        usage: dict = {
            "terminal_origin": "model_final",
            "final_text_integrity": {"degenerate": True},
        }
        event = {"type": "send_message", "task_id": "tt", "chat_id": 1, "text": corpus}
        prepare_terminal_send_event(
            tmp_path, {"id": "tt"}, corpus, usage, event,
            ephemeral=False, presence=False,
        )
        assert event["text"] == corpus  # delivery never mutates an answer

    def test_salvage_path_restates_clean_facts_and_keeps_text_intact(self, tmp_path):
        # >60-word all-unique text → clean facts re-derived, honest False
        # stamp; the preserved salvage copy keeps the answer byte-exact.
        clean_text = " ".join(f"w{i}" for i in range(80))
        usage: dict = {
            "terminal_origin": "host_salvage",
            "final_text_integrity": {"degenerate": True, "top_trigram_repeats": 27},
        }
        event = {"type": "send_message", "task_id": "ts", "chat_id": 1, "text": clean_text}
        prepare_terminal_send_event(
            tmp_path, {"id": "ts"}, clean_text, usage, event,
            ephemeral=False, presence=False,
        )
        assert (usage.get("final_text_integrity") or {}).get("degenerate") is False
        copies = [p for p in tmp_path.rglob("*") if p.is_file()]
        assert copies, "salvage copy should exist"
        assert any(clean_text in p.read_text(encoding="utf-8") for p in copies)
        assert event["text"] == clean_text  # delivered byte-exact
